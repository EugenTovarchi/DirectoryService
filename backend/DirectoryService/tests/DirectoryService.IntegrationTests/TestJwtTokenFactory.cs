...
public static class TestJwtTokenFactory
{
    ...
    private static string CreateToken(string? permission, DateTime notBefore, DateTime expires)
    {
        ...
        var signingCredentials = new SigningCredentials(
            new SymmetricSecurityKey(Encoding.UTF8.GetBytes(DirectoryTestWebFactory.TEST_JWT_SIGNING_KEY)),
            SecurityAlgorithms.HmacSha256);

        var token = new JwtSecurityToken(
            DirectoryTestWebFactory.TEST_JWT_ISSUER,
            DirectoryTestWebFactory.TEST_JWT_AUDIENCE,
            claims,
            notBefore,
            expires,
            signingCredentials);
