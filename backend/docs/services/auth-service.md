...
Последние проверки проходили: build `0 warnings / 0 errors`, refresh-token affected integration `8/8`, unit `7/7`, integration `23/23`.

Следующий ближайший AuthService блок: перенести legacy `/auth/users` registration slice на admin-created/invite flow или удалить после появления нового user management API.
