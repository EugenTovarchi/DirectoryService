...
- Refresh token session создается только через `RefreshToken.Create(...)`, чтобы доменная модель проверяла non-empty user id, корректную длину token hash и future expiration.
...
- Replacement refresh token session is created through `RefreshToken.Create(...)`; handlers do not call the entity constructor directly.
...
- Creates an inactive ASP.NET Core Identity `ApplicationUser` without a password.
- Assigns exactly one existing role.
- Sets `CurrentCompanyId` from request.
...
- Public endpoint; invite token itself is the secret.
- Accepts `AcceptInviteRequest` with `inviteToken` and `password`.
- Looks up only the hash of the raw invite token.
- Rejects unknown, expired, revoked, already accepted, already active, or already password-protected users with the same `invite.token.is.invalid` failure.
- Sets the Identity password, activates the user, marks the invite token accepted, and returns safe user fields with roles.
- Does not issue access/refresh tokens; after accepting invite the user signs in through `POST /api/auth/login`.
- Does not return password hash, security stamp, refresh tokens, token hashes, or session metadata.

Current `POST /api/users/{userId}/resend-invite` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Works only for inactive users without `PasswordHash`; active or password-protected users return `invite.resend.is.invalid`.
...
Current `GET /api/users/{userId}` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Returns one `CompanyUserDetailsResponse` for admin UI user details.
- Uses the same company boundary as `GET /api/users`: `SystemAdmin` can read any Identity user, while `CompanyAdmin` can read only users from their own `CurrentCompanyId`.
...
Current `PATCH /api/users/{userId}/profile` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Updates only safe profile fields; current slice supports `displayName`.
- `displayName: null` clears the display name.
- Does not change email, username, role, status, password, sessions, company context, or permissions.
...
- When deactivating a user, revokes all active refresh token sessions for that user.
- When activating a user, does not recreate or restore revoked sessions; the user must login again.
...
Current `PATCH /api/users/{userId}/change-role` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Accepts `ChangeUserRoleRequest` with `role`.
- Replaces the user's current role set with one existing role.
- Uses the same company boundary as user details: `SystemAdmin` can change any Identity user role, while `CompanyAdmin` can change only users from their own `CurrentCompanyId`.
- Returns `404 Not Found` for unknown users and for users outside a `CompanyAdmin` company boundary.
- Rejects self-role-change with `400 Bad Request` and AuthService-local `self.role.change.is.invalid`, so an admin cannot accidentally remove their own management permissions.
- Rejects `SystemAdmin` assignment by `CompanyAdmin` with `400 Bad Request` and AuthService-local `system.admin.role.assignment.is.invalid`; only `SystemAdmin` can assign the `SystemAdmin` role.
- Returns `CompanyUserDetailsResponse` after the role change.
- Command-side implementation uses Identity/EF Core through `UserManager<ApplicationUser>` and `RoleManager<ApplicationRole>`.
- Does not expose password hash, security stamp, concurrency stamp, refresh tokens, session metadata, or directory/tree access rules.

Current `POST /api/users/{userId}/revoke-sessions` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Revokes all active refresh token sessions for the target Identity user.
- Uses the same company boundary as user details: `SystemAdmin` can revoke sessions for any Identity user, while `CompanyAdmin` can revoke sessions only for users from their own `CurrentCompanyId`.
- Returns `404 Not Found` for unknown users and for users outside a `CompanyAdmin` company boundary.
- Rejects self-session-revocation with `400 Bad Request` and AuthService-local `self.session.revocation.is.invalid`; users should use `POST /api/auth/revoke-all-sessions` for their own sessions.
...
Refresh token session теперь создается через `RefreshToken.Create(...)`. План блока был заменить прямой constructor на фабрику для security-sensitive entity. Сделано: фабрика проверяет `UserId`, hash length и future expiration, login/refresh handlers используют фабрику, unit tests покрывают success и invalid cases. Влияние: некорректную refresh session сложнее случайно сохранить из application layer или тестового setup.

Первый `/api/users/invite` slice выбран как route для будущего invite lifecycle, но был реализован как admin-created Identity user без email delivery. План блока: дать CompanyAdmin/SystemAdmin управляемый способ создавать пользователей вместо legacy `/auth/users`. Сделано: endpoint требует policy `users.manage`, создает `ApplicationUser`, назначает роль и company context, временно принимал `InitialPassword`, а user-management failures получили явные AuthService-local codes. Влияние: AuthService получил первый Identity-based user management flow, после которого legacy учебный slice можно удалять отдельным блоком.
...
`PATCH /api/users/{userId}/change-status` добавлен как command-side status management. План блока: дать администратору возможность activate/deactivate пользователя из карточки, не добавляя role management. Сделано: endpoint требует `users.manage`, принимает `ChangeUserStatusRequest`, применяет company boundary как user details, запрещает self-deactivation, при deactivate отзывает active refresh sessions, меняет `ApplicationUser` через Identity/EF Core и возвращает `CompanyUserDetailsResponse`. Влияние: admin UI может включать и отключать учетные записи, а offboarding сразу блокирует refresh token lifecycle.

`PATCH /api/users/{userId}/change-role` добавлен как command-side role management. План блока: дать администратору возможность менять роль пользователя из карточки без изменения статуса и сессий. Сделано: endpoint требует `users.manage`, принимает `ChangeUserRoleRequest`, применяет company boundary как user details, запрещает self-role-change, не позволяет `CompanyAdmin` назначать `SystemAdmin`, заменяет текущий набор ролей на одну существующую роль через Identity/EF Core и возвращает `CompanyUserDetailsResponse`. Влияние: admin UI может повышать или понижать пользователя между MVP roles, не смешивая это с deactivate/fire сценариями.
...
Первый resource-service JWT slice доказал локальную проверку access token вне AuthService. План блока: подключить FileService и DirectoryService к текущим `issuer`/`audience`/symmetric signing parameters и защитить первые read endpoints через permission policies. Сделано: FileService требует `files.read` для media asset info, DirectoryService требует `directory.read` для roots/children, оба сервиса возвращают ожидаемые `401/403`, поддерживают Bearer auth в Swagger и покрыты integration tests с AuthService-compatible JWT. Влияние: permissions из ролей AuthService начали реально ограничивать downstream API без сетевого обращения к AuthService на каждый запрос.
