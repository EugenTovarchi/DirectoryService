...
- `fad9716` added refresh token rotation and reuse detection.
- `b22da61` added logout endpoint and renamed refresh slice to `RotateRefreshToken`.
- `c74646a` added revoke all sessions endpoint.
- `26f8bd0` fixed Roslyn package version conflict; AuthService build has no `NU1608`.
- `c9aa613` added current user sessions endpoint.
- `4f16f42` added revoke session endpoint and training notes for session lifecycle.
- `63c4e0a` documented SharedService clean JSON stdout backlog.
- latest block added `GET /api/auth/me` for current user, roles, permissions and company context.
...
Последние проверки проходили: build `0 warnings / 0 errors`, new current-user integration `2/2`, unit `1/1`, integration `23/23`.

Следующий ближайший AuthService блок: пересмотреть повторяющиеся auth errors после стабилизации login/refresh/logout/session/current-user flows.
