...
- `permission`: effective permissions;
- `jti`: access token id;
- `iss`: issuer;
- `aud`: audience;
- `exp`: expiration.
...
- Какой минимальный seed нужен для `SystemAdmin`, company, первого `CompanyAdmin`, roles и permissions?
- Какие integration tests пишем первыми: login/refresh или invite/accept?
