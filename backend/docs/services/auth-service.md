...
Nearest user-management MVP: use `POST /api/users/invite` as the public route name, but implement the first slice as admin-created/invite-created user setup without email delivery, raw invite token/link generation, or accept-invite flow. The route name is chosen now so the API can later grow into the full email invite lifecycle without renaming the user-management entry point.

Current `POST /api/users/invite` MVP behavior:

- Requires Bearer access token with `users.manage` permission.
- Creates an active ASP.NET Core Identity `ApplicationUser`.
...
- Request contains `InitialPassword`; this is temporary for admin-created local/dev flow and must be replaced by invite token acceptance when email invite lifecycle is implemented.
